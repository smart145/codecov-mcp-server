...
export class CodecovClient {
  ...
  constructor(apiToken: string, defaultService: Service = DEFAULT_SERVICE) {
    ...
    this.baseUrl = CODECOV_API_BASE_URL;
    const userAgent = process.env.CODECOV_USER_AGENT?.trim() || "codecov-mcp-server (mcp)";

    // Log the resolved base URL on startup for diagnostics.
    console.error(`[codecov-mcp] Using API base URL: ${this.baseUrl}`);
  ...
  private handleError(error: unknown): never {
    if (axios.isAxiosError(error)) {
      ...
      const requestUrl = axiosError.config?.url
        ? `${this.baseUrl}${axiosError.config.url}`
        : this.baseUrl;

      if (status === 401) {
        throw new CodecovApiError(
          `Authentication failed. Check your CODECOV_API_TOKEN. (URL: ${requestUrl})`,
          status,
          detail
        );
      }
      if (status === 403) {
        throw new CodecovApiError(
          `Access denied. You may not have permission to access this resource. (URL: ${requestUrl})`,
          status,
          detail
        );
      }
      if (status === 404) {
        throw new CodecovApiError(
          `Resource not found. Check the owner, repo, or path. (URL: ${requestUrl})`,
          status,
          detail
        );
      }
      if (status === 429) {
        throw new CodecovApiError(`Rate limit exceeded. Try again later. (URL: ${requestUrl})`, status, detail);
      }

      throw new CodecovApiError(
        `${detail || axiosError.message || "Unknown API error"} (URL: ${requestUrl})`,
        status,
        detail
      );
    }
  ...
  async getFileCoverage(
    ...
    try {
      ...
      // Encode path segments without encoding '/'.
      // Some servers/proxies reject or don't decode encoded slashes (%2F),
      // which can break file_report lookups on self-hosted installations.
