...
export function registerComparisonTools(server: McpServer): void {
  ...
  server.tool(
    ...
    "Compare coverage for a pull request (preferred) or between commits/branches. When pullid is provided, base/head are ignored. Shows coverage diff, changed files, and patch coverage. Essential for reviewing coverage impact of changes.",
    compareCoverageSchema.shape,
    async (args) => {
      try {
        const client = getCodecovClient();

        // Prefer pullid when provided; ignore base/head to avoid issues on
        // self-hosted instances that only support PR-based comparisons.
        const compareOptions = args.pullid
          ? { service: args.service as Service | undefined, pullid: args.pullid }
          : {
        ...
        const result = await client.compareCoverage(args.owner, args.repo, compareOptions);
